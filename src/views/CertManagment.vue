<template>
    <div class="certificante">
        ...
        <div class="form-group">
            ...
            <select id="province" v-model="selectedProvinces" multiple @change="onProvincesChanged">
                <option v-for="province in provinces" :key="province" :value="province">{{ province }}</option>
            </select>
            <div v-if="selectedProvinces.length" class="selected-box">
                <span v-for="province in selectedProvinces" :key="province" class="selected-item">{{ province }}</span>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        fetchData() {
            ...
            const queryString = this.selectedProvinces.join('&'); 
            fetch(`tu_endpoint_aqui?provinces=${queryString}`)
...
.certificante h1, .certificante h2 {
  color: #ecf0f1;
  margin-bottom: 1rem;
  font-weight: bold;
